...
parser.add_argument('--data_seed', default=0, type=int,
                    help='random seed data partitioning procedure')
...
# following MixMatch and FixMatch repo
...
#     print('weights used is {}'.format(weights))
    ...
    l_train_dataset = ImageCSVDataset(csv_file=args.l_train_dataset_path,
                                      root_dir=args.root_dataset_path,
                                      transform=TransformTwice(transform_labeledtrain))

    val_dataset = ImageCSVDataset(csv_file=args.val_dataset_path,
                                  root_dir=args.root_dataset_path,
                                  transform=transform_eval)

    test_dataset = ImageCSVDataset(csv_file=args.test_dataset_path,
                                   root_dir=args.root_dataset_path,
                                   transform=transform_eval)
    ...
    while total_used_time <= args.total_hour * 3600:  # run for 100 hour
        lr = sample_loguniform(low=-7, high=-3, size=1, coefficient=3, base=10)
        wd = sample_loguniform(low=-7, high=-3, size=1, coefficient=4, base=10)
        alpha = sample_uniform(low=0.1, high=10.0, size=1, decimal=1)

        print('!!!!!!!!!!This run using lr: {}, wd: {}!!!!!!!!!!'.format(lr, wd))

        hypercombo_iteratethrough_list.append({'lr': lr, 'wd': wd, 'alpha': alpha})  # newly added
        save_pickle(os.path.join(args.train_dir, 'global_stats'),
                    'hypercombo_iteratethrough_list.pkl', hypercombo_iteratethrough_list)
        ...
        experiment_name = "lr-{}_wd-{}_alpha-{}".format(args.lr, args.wd, args.alpha)
        ...
        l_loader = DataLoader(l_train_dataset, args.labeledtrain_batchsize,
...
#                 best_train_raw_acc_at_val = 0
    ...
    #             best_train_raw_acc_at_val = checkpoint['best_train_raw_acc_at_val']
        ...
        for epoch in range(args.start_epoch, args.train_epoch):
            ...
            # train
            train_labeled_loss_list = train_one_epoch(
                args, weights, l_loader, model, optimizer, scheduler, epoch)
            ...
            logger.info('RAW Best , validation/test/train %.2f %.2f' %
        ...
        hypercombo_iteratethrough_time_list.append(
            this_hypercombo_iteratethrough_time)  # newly added
        save_pickle(os.path.join(args.train_dir, 'global_stats'),
                    'hypercombo_iteratethrough_time_list.pkl', hypercombo_iteratethrough_time_list)
        ...
        global_best_test_raw_acc_at_val_list_parallel.append(
            this_hypercombo_best_test_raw_acc_at_val_list_parallel)

        save_pickle(os.path.join(args.train_dir, 'global_stats'),
                    'global_best_val_raw_acc_list_parallel.pkl', global_best_val_raw_acc_list_parallel)
        save_pickle(os.path.join(args.train_dir, 'global_stats'),
                    'global_best_test_raw_acc_at_val_list_parallel.pkl', global_best_test_raw_acc_at_val_list_parallel)
