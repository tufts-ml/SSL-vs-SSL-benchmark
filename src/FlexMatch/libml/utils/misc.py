...
#     print('inital classwise_acc: {}'.format(classwise_acc))
    #############################unique to FlexMatch compared to FixMatch#############################
    
    for batch_idx in range(n_steps_per_epoch):
 
        try:
            _, l_input, l_labels = labeledtrain_iter.next() #unique to FlexMatch compared to FixMatch: also give idx
        except:
            labeledtrain_iter = iter(labeledtrain_loader)
            _, l_input, l_labels = labeledtrain_iter.next()
        
        try:
            u_idx, (u_input_weak, u_input_strong), u_labels = unlabeledtrain_iter.next() #unique to FlexMatch compared to FixMatch: also give idx
        except:
            unlabeledtrain_iter = iter(unlabeledtrain_loader)
...
#             print('threshold warmup executed')
